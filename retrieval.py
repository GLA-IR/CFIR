...
from atomic.atomic_dataset import create_answer_dataset, create_downstream_dataset, create_query_answer_dataset
from atomic.atomic_engine import evaluate_onalldata
...
from atomic.atomic_engine import train_one_epoch, get_handler, evaluate
...
def get_args():
    ...
    parser.add_argument('--task', type=str, required=True,
                        choices=['atomic', 'atomic_allret_adapter', 'atomic_stage1', 'atomic_stage2'],
    ...
    parser.add_argument('--use_summary', action='store_true', default=False, )
    parser.add_argument('--save_embeddings_to_npy', action='store_true', default=False, )
    #
    ...
    parser.add_argument('--load_image_from_precomputed_npy', action='store_true', default=False, )
    parser.add_argument('--embeddings_file_path', type=str, default=None, help='')
    parser.add_argument('--query_url', type=str, default=None, help='')
    parser.add_argument('--produce_stage1_candidates', action='store_true', default=False, )
    parser.add_argument('--load_stage1_candidates', action='store_true', default=False, )
    ...
    parser.add_argument('--ffn_adapter_scalar', default="0.3", type=str,
    ...
    parser.add_argument('--tune_text_only', default=False, action='store_true', help='only tune text expert')
...
def main(args, ds_init):
    ...
    if args.eval:
        data_loader_train, data_loader_val = create_query_answer_dataset(args)
    else:
        data_loader_train, _ = create_downstream_dataset(args)
        data_loader_val = create_downstream_dataset(args, is_eval=True)

    #  set output dir
    if args.eval:
        exp_tag = f"eval_{args.num_max_bpe_tokens}tokens_{args.input_size}imageres_{args.finetune.split('/')[-1]}/"
    else:
        exp_tag = f"{args.tune_text_only}_textonly_{args.model}_{args.epochs}epochs_{args.num_max_bpe_tokens}tokens_{args.use_entity}_entity_{args.use_summary}_summary_{args.ffn_mode}mode_{args.ffn_num}ffn_num_{args.ffn_adapter_scalar}raito_{args.seed}seed_{args.task}tasks_{args.finetune.split('/')[-1]}/"
    ...
    if not args.model.endswith(args.task):
        if args.task in ("atomic"):
            model_config = "%s_retrieval" % args.model
        elif args.task in ("atomic_allret_adapter", "atomic_stage1", "atomic_stage2"):
            model_config = "%s_retrieval" % args.model
        else:
    ...
    if args.tune_text_only:
        for name, p in model.named_parameters():
            if 'text_embed' in name or 'text' in name:
                p.requires_grad = True
            elif 'ffn.B' in name or 'final_layer_norm.B' in name:
                p.requires_grad = True
            else:
    ...
    # build all data of atomic
    if not args.eval_on_test_set:
    ...
    if args.eval:
        ...
        if args.build_ranking:
            ext_test_stats, task_key, text_to_image_rank = evaluate_onalldata(query_dataloader, answer_dataloader, model, device, task_handler, args, build_ranking=True)
        else:
            ext_test_stats, task_key = evaluate_onalldata(query_dataloader, answer_dataloader, model, device, task_handler, args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.eval_on_test_set and (epoch + 1) % args.eval_freq == 0:

            test_stats, task_key = evaluate_onalldata(data_loader_val, answer_dataloader, model, device, task_handler, args)
